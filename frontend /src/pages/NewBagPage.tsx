...
const NewBagPage = () => {
  ...
  //     if (heightRef.current && !heightRef.current.contains(e.target as Node)) {
  //       setHeightOpen(false);
  //     }
  //   };
  ...
  const isLoading = clubsLoading || collectionsLoading;
  const categories = [
    ...(collectionsData ?? []),
  ...
  const handleSaveBag = () => {
    ...
    const name =
      bagName.trim() ||
      `My Bag — ${now.toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
    ...
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-golf-yellow text-sm mt-1">
          Select clubs and save them as a named bag.
        </p>
      </div>
          ...
          <div className="bg-white/10 rounded-lg border border-white/20 overflow-hidden">
            ...
            {selectedClubs.length === 0 ? (
              <div className="px-4 py-4 lg:py-6 flex items-center gap-3 lg:flex-col lg:text-center">
                <svg
                  className="w-7 h-7 lg:w-8 lg:h-8 text-golf-yellow lg:mx-auto lg:mb-2 flex-shrink-0"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
