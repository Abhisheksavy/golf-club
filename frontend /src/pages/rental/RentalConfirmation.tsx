    ...
    <div className="max-w-lg mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm">
          {course ? `Your clubs are reserved at ${course.name}.` : "Your reservation has been confirmed."}
        </p>
      </div>
        ...
        {course && (
          <div className="px-5 py-4">
            ...
            {(course.address || course.location) && (
              <p className="text-sm text-gray-500">{course.address ?? course.location}</p>
            )}
          </div>
        )}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
        <button onClick={handleViewReservations} className="btn-primary w-full sm:w-auto">
          View My Reservations
        </button>
        <button onClick={handleNewReservation} className="btn-secondary w-full sm:w-auto">
          Make Another Reservation
        </button>
        <button onClick={handleDashboard} className="btn-secondary w-full sm:w-auto">
          Dashboard
        </button>
      </div>
    </div>
